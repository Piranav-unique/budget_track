/**
 * Standalone TypeScript script to test email availability and SMTP configuration
 * 
 * Usage:
 *   npx ts-node scripts/test-email.ts
 *   npx ts-node scripts/test-email.ts --send test@example.com
 * 
 ...
 */
function checkConfiguration(): SMTPConfig | null {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (!value && key !== 'SMTP_FROM' && key !== 'SMTP_PORT') {
            allConfigured = false;
        }
    }
 ...
 */
async function testConnection(config: SMTPConfig): Promise<nodemailer.Transporter | null> {
    logSection('🔌 Testing SMTP Connection');

    if (!config.SMTP_HOST || !config.SMTP_USER || !config.SMTP_PASS) {
        log('  ❌ Missing required configuration', 'red');
        return null;
    }
 ...
 * Send a test email
 */
async function sendTestEmail(transporter: nodemailer.Transporter, toEmail: string): Promise<boolean> {
...
async function main(): Promise<void> {
    ...
    if (sendEmailIndex !== -1 && args[sendEmailIndex + 1]) {
        ...
        log('   npx ts-node scripts/test-email.ts --send your-email@example.com', 'yellow');
        process.exit(0);
    }
}
